...
*Setup instructions for a Gitian build of Elysium Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Elysium
...
- [Building Elysium Core](#building-elysium-core)
...
Clone the git repositories for elysium and Gitian.
...
in the elysium repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Elysium Core
----------------

To build Elysium Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the elysium repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/elysium/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/elysium-project/elysium
...
For example:
```bash
URL=https://github.com/thrasher-/elysium.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit elysium=${COMMIT} --url elysium=${URL} ../elysium/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit elysium=${COMMIT} --url elysium=${URL} ../elysium/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit elysium=${COMMIT} --url elysium=${URL} ../elysium/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the elysium git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../elysium/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/elysium
SIGPATH=/some/root/path/elysium-detached-sigs

./bin/gbuild --url elysium=${BTCPATH},signature=${SIGPATH} ../elysium/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    ...
    gpg --detach-sign ${VERSION}-win/${SIGNER}/elysium-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/elysium-osx-build.assert
```
