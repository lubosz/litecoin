...
By default, Gitian will fetch source files as needed. To cache them ahead of time:
    ...
    pushd ./gitian-builder
    make -C ../elysium/depends download SOURCES_PATH=`pwd`/cache/common
    popd
...
NOTE: Offline builds must use the --url flag to ensure Gitian fetches only from local URLs. For example:
    ...
    pushd ./gitian-builder
    ./bin/gbuild --url elysium=/path/to/elysium,signature=/path/to/sigs {rest of arguments}
    popd
...
Build output expected:
  ...
  1. source tarball (`elysium-${VERSION}.tar.gz`)
...
Add other gitian builders keys to your gpg keyring
    ...
    gpg --import elysium/contrib/gitian-keys/*.pgp
...
- Detached signatures will then be committed to the [elysium-detached-sigs](https://github.com/elysium-project/elysium-detached-sigs) repository, which can be combined with the unsigned apps to create signed binaries.
...
```
elysium-${VERSION}-aarch64-linux-gnu.tar.gz
elysium-${VERSION}-arm-linux-gnueabihf.tar.gz
elysium-${VERSION}-i686-pc-linux-gnu.tar.gz
elysium-${VERSION}-x86_64-linux-gnu.tar.gz
elysium-${VERSION}-osx64.tar.gz
elysium-${VERSION}-osx.dmg
elysium-${VERSION}.tar.gz
elysium-${VERSION}-win32-setup.exe
elysium-${VERSION}-win32.zip
elysium-${VERSION}-win64-setup.exe
elysium-${VERSION}-win64.zip
```
The `*-debug*` files generated by the gitian build contain debug symbols
for troubleshooting by developers. It is assumed that anyone that is interested
in debugging can run gitian to generate the files for themselves. To avoid
end-user confusion about which file to pick, as well as save storage
space *do not upload these to the elysium.org server, nor put them in the torrent*.
...
- Upload zips and installers, as well as `SHA256SUMS.asc` from last step, to the elysium.org server.

- Update elysium.org version

- Announce the release:

  - elysium-dev mailing list

  - Elysium Core announcements list https://groups.google.com/forum/#!forum/elysium-dev

  - blog.elysium.org blog post

  - elysiumtalk.io forum announcement

  - Update title of #elysium on Freenode IRC
